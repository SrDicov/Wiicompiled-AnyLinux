...
# or absent (Alpine/NixOS: ENOENT). Fix: replace the sharun-hardlink bin/bash
# with a tiny host-sh wrapper that re-execs the real bash through the BUNDLED
# loader explicitly - then bash READS the script (no re-resolution) on every
# host. #!/bin/sh needs nothing: no image sh exists, it always host-resolves.
...
# The wrapper locates its own image WITHOUT trusting $APPDIR: nested tooling
# (package-game-appimage.sh) legitimately re-exports APPDIR for the AppDir it
# is building, and an APPDIR-trusting wrapper died on that (CI synthetic
# test: setup wrapper execing /tmp/wii-game-pack-*/AppDir/lib/ld-linux).
...
echo "Wrapping bin/bash for musl hosts (loader: $_ld_name)..."
cat > "$appdir/bin/.bash.wrapper" <<'WRAPPER_EOF'
#!/bin/sh
# Generated by build-appimage.sh (musl-host fix, see comment there): host sh
# (exists on every Linux) re-execs the bundled bash through the bundled
# loader, so script args never go through sharun shebang resolution (fatal
# on musl). Locates its own image WITHOUT trusting $APPDIR (nested tooling
# re-exports it): $0 with a slash is our path, otherwise the first bash on
# PATH (in-image runs always list exactly one image bin dir first).
case "$0" in
    */*) _bd=$(dirname "$0") ;;
    *) _bd="" ;;
esac
if [ -z "$_bd" ]; then
    ...
    for _d in $PATH; do
        IFS=$_oifs
        if [ -n "$_d" ] && [ -x "$_d/bash" ]; then _bd=$_d; break; fi
    done
    IFS=$_oifs
fi
...
[[ "$(head -c 4 "$appdir/bin/bash")" != $'\x7fELF' ]] || {
    echo "build-appimage.sh: error: bin/bash is still an ELF (sharun farm)" >&2; exit 1; }
