...
#!/bin/sh
# Generated by build-appimage.sh (musl-host fix, see comment there):
# host sh (exists on every Linux) re-execs the bundled bash through the
# bundled loader, so script args never go through sharun's shebang
# resolution (which execs shared/bin/* directly - fatal on musl).
: "\${APPDIR:?wiicompiled bash wrapper needs APPDIR from AppRun.sh}"
exec "\$APPDIR/lib/$_ld_name" --library-path "\$APPDIR/lib" "\$APPDIR/shared/bin/bash" "\$@"
WRAPPER_EOF
